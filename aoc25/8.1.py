...
def parse_boxes() -> list[Box]:
    with open("8.1.txt", 'r') as data_file:
...
def get_circuit_lengths_descending(boxes: list[Box]):
    all_circuit_ids = [box.circuit_id for box in boxes]
    counter = Counter(all_circuit_ids)
    return list(sorted(counter.values(), reverse=True))
...
def solve():
    ...
    for connection in sorted_connections[:1000]:
        if connection.box_a.circuit_id == connection.box_b.circuit_id:
            continue

        connect_boxes(BOXES, connection)

    circuit_lengths = get_circuit_lengths_descending(BOXES)

    if len(circuit_lengths) < 3:
        raise Exception("Somehow not enough circuits")

    a, b, c = circuit_lengths[:3]
